...
# Scenario 8: x1i (binary), x2i (uniform), x3ij (normal) covariates
...
get_alpha <- function(tau, beta0, beta1, beta2, beta3, lambda) {
  alpha0 <- log(-log(1 - tau) / lambda) + beta0
  c(alpha0, beta1, beta2, beta3)
}

# Parameters
beta0 <- 0.6; beta1 <- 0.6; beta2 <- 0.8; beta3 <- 0.4
lambda <- 0.69
M <- 1e6
n_cluster <- c(200, 500)
size_cluster <- c(1, 3, 10)
tau_vals <- c(0.25, 0.5, 0.75) # Originally 0.5 only
Ktau_vals_all <- c(0, 0.5, 0.8)
t0_vals <- c(0, 1, 2)
...
for (n in n_cluster) {
  for (m in size_cluster) {
    for (tau in tau_vals) {
      for (t0 in t0_vals) {
        Ktau_vals <- if (m == 1) c(0) else Ktau_vals_all
        for (Ktau in Ktau_vals) {
          
            theta <- 2 * Ktau / (1 - Ktau)
            scenario_key <- paste0("n", n, "_m", m, "_tau", tau, "_Ktau", Ktau, "_t0", t0)
            
            all_data[[scenario_key]] <- vector(mode = "list", length = n_sim)
            cumulative_censoring <- numeric(n_sim)
            
            for (b in 1:n_sim) {
              # Generate covariates
              x1i <- rbinom(n, size = 1, prob = 0.5)
              x2i <- runif(n)
              x3ij <- matrix(rnorm(n * m), nrow = n, ncol = m)
              
              # Expand x1i and x2i across clusters
              x1 <- matrix(rep(x1i, each = m), nrow = n, byrow = TRUE)
              x2 <- matrix(rep(x2i, each = m), nrow = n, byrow = TRUE)
              
              # Total covariate effect
              cov_effect <- beta1 * x1 + beta2 * x2 + beta3 * x3ij
              
              # Error generation
              if (m == 1) {
                u <- matrix(runif(n * m), nrow = n)
              } else {
              ...
              alpha <- get_alpha(tau, beta0, beta1, beta2, beta3, lambda)
              logT_raw <- alpha[1] + cov_effect + eps_raw
              T_raw <- t0 + exp(logT_raw)
              
              eps_cond <- eps_raw[T_raw > t0]
              q_cond_tau <- quantile(eps_cond, probs = tau)
              eps <- eps_raw - q_cond_tau
              
              logT <- alpha[1] + cov_effect + eps
              T <- t0 + exp(logT)
...
save(all_data, file = "data_scenario8.RData") # saving the data_list for other simulations
...
load("data_scenario8.RData")  # Load pre-generated data set
...
for (scenario_key in names(all_data)) {
  ...
  est_matrix <- matrix(NA, nrow = n_sim, ncol = 16)  # Now stores both methods
  censoring_vec <- numeric(n_sim)
  
  # Get true alpha values for this scenario
  truth <- get_alpha(tau_val, beta0, beta1, beta2, beta3, lambda)
  ...
  for (b in 1:n_sim) {
    ...
    # qris: non-smooth
    fit_ns <- qris(Surv(obs_time, delta) ~ x1 + x2 + x3, data = df, t0 = t0_val, Q = tau_val, nB = 0, method = "nonsmooth")
    est_matrix[b, 9:12] <- fit_ns$coefficient
    
    # qris: smooth
    fit_is <- qris(Surv(obs_time, delta) ~ x1 + x2 + x3, data = df, t0 = t0_val, Q = tau_val, nB = 0, method = "smooth")
    est_matrix[b, 13:16] <- fit_is$coefficient
    ...
    U_mat <- cbind(1, df$x1, df$x2, df$x3) # Build U matrix
    ...
    est_matrix[b, 1:4] <- fit$coefficients  # Store fit_non_smooth results
    ...
    if (is.fit$termcd %in% c(1, 2)) {
      est_matrix[b, 5:8] <- is.fit$x  # Store induced smoothing results
    }
    
    if (b %% (n_sim) == 0) {
      mean_fit_ns <- colMeans(est_matrix[1:b, 1:4, drop = FALSE], na.rm = TRUE)
      mean_is <- colMeans(est_matrix[1:b, 5:8, drop = FALSE], na.rm = TRUE)
      mean_ns_qris <- colMeans(est_matrix[1:b, 9:12, drop = FALSE], na.rm = TRUE)
      mean_is_qris <- colMeans(est_matrix[1:b, 13:16, drop = FALSE], na.rm = TRUE)
  ...
  mean_fit_ns <- colMeans(est_matrix[, 1:4, drop = FALSE], na.rm = TRUE)
  mean_is <- colMeans(est_matrix[, 5:8, drop = FALSE], na.rm = TRUE)
  mean_ns_qris <- colMeans(est_matrix[, 9:12, drop = FALSE], na.rm = TRUE)
  mean_is_qris <- colMeans(est_matrix[, 13:16, drop = FALSE], na.rm = TRUE)
  ...
  results[[scenario_key]] <- data.frame(
      Coef = rep(c("alpha0", "alpha1", "alpha2", "alpha3"), 4),
    Method = rep(c("fit_non_smooth", "qris_nonsmooth", "induced_smoothing", "qris_smooth"), each = 4),
...
write.csv(results_df, "results_ns_vs_is_all(scenario8).csv", row.names = FALSE)

# (2) All estimates compiled: all_estimates
est_long <- bind_rows(lapply(names(all_estimates), function(k) {
  df <- all_estimates[[k]]
  colnames(df) <- c("fitNS_0", "fitNS_1", "fitNS_2", "fitNS_3",
                    "IS_0", "IS_1", "IS_2", "IS_3",
                    "qrisNS_0", "qrisNS_1", "qrisNS_2", "qrisNS_3",
                    "qrisIS_0", "qrisIS_1", "qrisIS_2", "qrisIS_3")
  df$Scenario <- k
...
est_longer <- est_long %>%
  pivot_longer(
    cols = starts_with("fitNS_") | starts_with("IS_") | starts_with("qrisNS_") | starts_with("qrisIS_"),
  ...
  mutate(
    ...
    Coef = recode(CoefNum, `0` = "alpha0", `1` = "alpha1", `2` = "alpha2", `3` = "alpha3")
  )
...
est_longer <- est_longer %>%
  mutate(True_vals = case_when(
    Coef == "alpha0" ~ log(-log(1 - tau) / lambda) + beta0,
    Coef == "alpha1" ~ beta1,
    Coef == "alpha2" ~ beta2,
    Coef == "alpha3" ~ beta3
  ))
...
coefs <- c("alpha0", "alpha1", "alpha2", "alpha3")
...
plot_boxp_trimmed <- function(data, coef_name, cluster_size_val, tau_val, n_val, iqr_mult = 3) {
  # Filter relevant data
  df <- data %>%
    filter(Coef == coef_name, m == cluster_size_val, tau == tau_val, n == n_val)
  
  # Remove extreme outliers within each Method × Ktau × t0 group
  df_trimmed <- df %>%
    group_by(Method, Ktau, t0) %>%
    mutate(
      Q1 = quantile(Estimate, 0.01, na.rm = TRUE),
      Q3 = quantile(Estimate, 0.99, na.rm = TRUE),
      IQR = Q3 - Q1,
...
plot_boxp_trimmed(est_longer, coef_name = "alpha1", cluster_size_val = 1, tau_val = 0.75, n_val = 200)
